...
The ``modelprovider`` enum carries ``'openrouter'`` as of migration 094.
...
Revision ID: 095_seed_openrouter_provider
Revises: 094_modelprovider_openrouter
...
revision = "095_seed_openrouter_provider"
down_revision = "094_modelprovider_openrouter"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    ...
    op.execute(
        sa.text("DELETE FROM provider_configs WHERE name = 'OpenRouter'")
    )