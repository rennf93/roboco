"""Add 'nebius' to the modelprovider enum.

Mirrors migration 095 (openrouter): adds ``'nebius'`` to the PostgreSQL
``modelprovider`` enum so ``ProviderConfigTable.type`` accepts
``ModelProvider.NEBIUS``. The ``ALTER TYPE ... ADD VALUE IF NOT EXISTS``
...
Revision ID: 099_modelprovider_nebius
...
revision = "099_modelprovider_nebius"
down_revision = "098_seed_zai_provider"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # using 'nebius' (Postgres forbids using a freshly added enum value
    ...
    with op.get_context().autocommit_block():
        op.execute("ALTER TYPE modelprovider ADD VALUE IF NOT EXISTS 'nebius'")
