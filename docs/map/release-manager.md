...
**Window boundary + FE contract (pr_gate findings F-2cadcce6 / F-b7ba8602 / F-a35e595f, task `f08043e3`).** The window boundary previously resolved to `None` in production: approve()'s publish path set `status=COMPLETED` without stamping `completed_at`, and Postgres `DESC` = NULLS FIRST meant the NULL row won `LIMIT 1`, dropping the time filter entirely (every COMPLETED delivery task in project history became a member). The publish path now stamps `completed_at` in the same commit that flips COMPLETED, the previous-release lookup carries a `completed_at IS NOT NULL` guard, and legacy (pre-stamp) proposals fall back to `updated_at` (written by that same COMPLETED commit via ORM onupdate) as the boundary. Members are returned in a total order (`completed_at` ASC, NULLS LAST, `id` tiebreak) so repeated proposal polls never reshuffle the panel rollup. The `{task_id, pr_number}` object shape is the confirmed FE contract: the panel's `ReleaseMemberTaskId` (`panel/src/lib/api/release.ts`, PR #1069) types `member_task_ids: ReleaseMemberTaskId[]` and reads `m.task_id` per member; the `member_task_ids` name is kept for FE stability even though it carries objects.
