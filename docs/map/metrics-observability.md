...
- **Panel surface**: `panel/src/components/metrics/zero-progress-spawn-waste-card.tsx`'s `ZeroProgressSpawnWasteCard` (Metrics page → Delivery tab, its own full-width row between the ReworkCard/ProvenanceCard row and the per-cell scorecards) is the only panel consumer of `GET /api/dashboard/metrics/spawn-waste`: it calls `useZeroProgressSpawnWaste(30)` (plumbing leaf PR, see `docs/frontend/hooks.md`) and renders the report's own `zero_progress_cost_share` as the headline number plus a "N of M sessions advanced nothing · $X of $Y spawn cost" line, per-team badges (`zero_progress_sessions/sessions · cost · rate`), and a by-agent `ResponsiveTable` (top 8, card-list fallback on narrow viewports — same pattern as `ReworkCard`). Skeleton/empty/error states match the tab's other cards (one-line "Failed to load spawn-waste metrics."). The title "Zero-Progress Spawn Sessions (30d)" and the HelpTip deliberately spell out the zero-progress definition and that this is NOT the Metrics page's usage-based "Spawn Waste" card (`GET /usage/spawn-waste`, zero output tokens) — that endpoint collision is the Gotcha below. `by_task[]` is served by the report but not surfaced on the card.
...
>
> **Zero-progress spawn-waste card on the Delivery tab (2026-08-31, PR #984 + sibling plumbing leaf b0b1fba3).** The dashboard spawn-waste endpoint finally got its panel consumer: plumbing leaf b0b1fba3 landed `observabilityApi.getSpawnWaste(days)` (`panel/src/lib/api/observability.ts`), `useZeroProgressSpawnWaste(days = 30)` (`panel/src/hooks/use-observability.ts`), and `SpawnWasteReport`/`AgentSpawnWasteRow`/`TeamSpawnWasteRow` types (`panel/src/types/index.ts`) with a drift-guard test pinning the literal `/dashboard/metrics/spawn-waste` path; card leaf PR #984 then added `panel/src/components/metrics/zero-progress-spawn-waste-card.tsx`'s `ZeroProgressSpawnWasteCard` — headline `zero_progress_cost_share`, "N of M sessions advanced nothing · $X of $Y spawn cost", by-team badges, by-agent `ResponsiveTable` (ReworkCard's pattern, top 8, card-list fallback), Skeleton/empty/error states — wired into `DeliveryTabContent` between the Rework/Provenance row and the per-cell scorecards (all changes confined to `panel/src/components/metrics/` including a new focused `__tests__/zero-progress-spawn-waste-card.test.tsx` that mocks the hook à la `portfolio-cards.test.tsx`, plus a `useZeroProgressSpawnWaste` mock added to the existing `delivery-tab.test.tsx`). Title and HelpTip copy state the zero-progress definition and the NOT-the-usage-card distinction in the UI itself; `by_task[]` is served but deliberately not surfaced on the card.
