...
| `MetricsService.get_portfolio_metrics` | method | metrics.py:1463 | Per-project portfolio aggregation grouped by `tasks.project_id` — 4 slices: active-task count (statuses minus `_PORTFOLIO_HELD_STATUSES`), median lead time + rework rate from ONE windowed completed-tasks pass (same population; median over mean because agent lead times are heavy-tailed), open-findings count SQL-grouped from `TaskReviewFindingTable` (deliberately not the capped `ReviewFindingsRepository.list_open_findings`), and calendar-month spawn burn. Backs `GET /api/dashboard/portfolio` — see `docs/backend/api/dashboard-portfolio.md`. |
| `MetricsService._portfolio_spend_by_project` | func | metrics.py:145 | Month-to-date agent-spawn burn per project in ONE query: `AgentSpawnSessionTable` joined to `TaskTable` (string-cast id), sessions started ≥ 1st of month UTC; closed sessions use stored `estimated_cost_usd`, open ones priced live via `calculate_cost` — mirrors `TaskService.project_month_spend_usd`'s pricing, batched for the whole fleet. |
| `_PORTFOLIO_HELD_STATUSES` | const | metrics.py:113 | `{BACKLOG, COMPLETED, CANCELLED}` — statuses excluded from the portfolio's "active" count. Backlog is parked, not active; null-project tasks never appear (grouped BY project). |
...
| `DashboardService.get_portfolio` | method | dashboard.py:284 | Merges project identities onto `MetricsService.get_portfolio_metrics`' per-project dict so EVERY project appears once (zeroed when inactive), sorts most-active-first — backs `GET /api/dashboard/portfolio` (see `docs/backend/api/dashboard-portfolio.md`). |
