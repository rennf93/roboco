...
**Internal (roboco):**
- `roboco.db.tables` — `AgentSpawnSessionTable`, `DailyUsageRollupTable`, `AgentTable`, `AuditLogTable`, `TaskTable`, `NotificationTable`, `VerbLatencySampleTable`
- `roboco.db.base` — `get_session_factory` (separate-session telemetry write path)
- `roboco.api.middleware` — `RequestLoggingMiddleware` (telemetry persistence seam), `FlowVerbTimeoutMiddleware` (504-timeout classification)
- `roboco.foundation.policy.flow_timeouts` — `SLOW_VERBS` (timeout-breach flag threshold in Sentinel latency section)
