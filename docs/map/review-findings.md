...
**A fifth, advisory producer (2026-09-05, round-1 pr_gate hardened): `origin="second_review"`.** Unlike the four bounce producers above, the flag-gated cross-vendor second-review pass (`roboco/services/gateway/choreographer/second_review_gate.py`, wired into `pr_pass` — see `docs/map/pr-gate-review.md`) inserts findings WITHOUT transitioning the task to `needs_revision` and without any of the bounce machinery (no `revision_count` bump, no `task.*` rejection audit event, no structured verdict note) — it fires at `pr_pass` time, i.e. on a task the gate is already passing forward. It reuses the same `findings_lib.insert_and_render` chokepoint every other producer shares, so a `second_review`-origin row has the identical per-finding shape and renders in the same panel Findings tab. Unlike the other four producers, though, `insert_second_review_findings` stamps every inserted row `STATUS_ADDRESSED` right after the shared `insert_and_render` call, same transaction, before its own flush — never observably `open`: this origin lands on an already-ASSEMBLED cell/root task, which has no dev claim cycle to route an `open` finding's resolution through — an `open` row here would sit unresolvable forever and wrongly block every LATER gate on this same task_id (`complete`, `ceo_approve`, a future resubmit after an unrelated bounce, ...) via the origin-agnostic `_open_finding_ids`/`FINDINGS_ADDRESSED` check (which reads OPEN findings task-wide, not per-origin). There is still no `origin="second_review"` call site for `stamp_addressed_verified` (unlike `qa`/`pr_gate`/`pm`/`ceo`, each verified by its own next-round pass verb) — a `second_review` finding never advances past `addressed` to `verified`. This is deliberate, not a bug: it never needs to be named in `resolved_findings` either, since it was never `open` in the first place.
...
│   └── _run_second_review_pass (pr_gate.py + second_review_gate.py, origin=second_review) -> advisory, no transition/bump/verdict-note; inserts pre-STATUS_ADDRESSED, no verify-stamp call site (see Gotchas)
├── Resolution
...
- `origin="second_review"` findings are inserted then immediately stamped `STATUS_ADDRESSED` (`insert_second_review_findings` mutates the just-inserted row objects and flushes, same transaction — `insert_and_render`/`insert_many` themselves are unchanged and still leave every OTHER caller's rows at the shared `open` default) and have no `stamp_addressed_verified` call site anywhere — so this origin never reaches `verified` either. That's deliberate: every other origin gets verified by its own next-round pass verb (`pass_review`/`pr_pass`/`complete`/`ceo_approve`), but `second_review` has no equivalent "pass" verb to hook (it's an advisory ledger insert on an already-assembled task, not a bounce with a dev to resolve it) — and it never needs `resolved_findings` naming either, since it was never `open`. Don't expect a `verified` status on this origin in any panel view or metrics query.
