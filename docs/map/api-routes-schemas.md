...
| GET | /api/dashboard/portfolio?days= | dashboard.py | agent context + **explicit CEO-role 403 guard** (the first dashboard route with a router-level role check, not just service-layer filtering) — per-project portfolio rows over `DashboardService.get_portfolio` + `MetricsService.get_portfolio_metrics`; see `docs/backend/api/dashboard-portfolio.md` |
