...
| roboco/api/auth/ | Cloud auth (FastAPI Users, default off): `backend.py` (cookie transport + password-fingerprint-bound JWT strategy), `manager.py` (`UserManager` + DI chain), `session.py` (`resolve_session_user`, shared by the HTTP dual-path and the WS panel-token gate), `seed.py` (idempotent single seeded CEO login upsert), `routes.py` (always-public `/auth/status` + conditional login/logout mount), `login_limit.py` (`LoginRateLimiter` — per-IP POST rate limit). |
...
- The `POST /api/telegram/webapp-auth` Mini App sign-in route and its supporting `/api/telegram/*` surfaces were stripped in PR #640 and are no longer mounted by `app.py`.
...
> - PR #640 (2026-07-21) stripped the Telegram Mini App V3/V4 backend routes: `POST /api/telegram/webapp-auth`, `GET /api/telegram/today`, and the `/api/telegram/credentials` surface are no longer mounted or described here. `LoginRateLimiter` was simplified back to a single-path shape when the webapp-auth bucket was removed.
