...
| `resolve_second_review_provider` | func | `services/second_review.py:113` | Pure resolver: first enabled provider differing from `authoring_provider`, in caller-supplied order; never raises — returns `SecondReviewSelection.skip(...)` with an evidence note when no differing enabled provider exists (e.g. single-vendor fleet) |
...
**Cross-vendor second review (not yet wired).** `SecondReviewService.resolve_second_reviewer_for_agent` calls `ModelRoutingService(session).resolve_for_agent` to get the authoring dev/QA chain's `ModelProvider`, then `enabled_providers()` (via `ProviderService.list_providers(include_disabled=False)`) to get the fleet's currently-enabled provider set, and delegates to the pure `resolve_second_review_provider` to pick a differing one (or an explicit non-raising skip). `task_is_high_stakes`/`is_high_stakes` independently gate whether a task even qualifies, purely off `Settings` + task signals — the flag defaults off, so both paths are currently dormant in production. This module is a standalone service with no caller yet in this codebase; a sibling gate-wiring task (the in-path PR gate, `pr_gate.py`) is expected to call it from `_pr_pass_blocked` or a new preflight step. See `docs/map/pr-gate-review.md` for the gate this is meant to feed.
...
- **`second_review.py` has no caller yet** — `SecondReviewSelection` / `resolve_second_review_provider` / `SecondReviewService` are complete and unit-tested (`tests/unit/services/test_second_review.py`), but no route or gateway verb invokes `SecondReviewService` yet; `cross_vendor_review_enabled` defaults `False` so the classifier is inert either way. Don't assume a high-stakes task actually gets a differing second reviewer today — that requires a sibling task to wire this into the PR gate (`pr_gate.py`).
