...
- ROBOCO_PROVISIONING_ENABLED / ROBOCO_PROVISIONING_TOKEN / ROBOCO_PROVISIONING_ORG / ROBOCO_GITHUB_API_BASE_URL — retained in config for backward compatibility, but PR #640 removed the pitch approve path that consumed them; pitch approval now raises `ProvisioningDisabledError` regardless of these values
