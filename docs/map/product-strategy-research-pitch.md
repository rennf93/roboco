...
| `roboco/services/strategy_engine.py` | Dormant "engine 2": assesses company state vs goals; `idle`→roadmap and `stranded_blocked`→Coroner both trigger a Board Program before the CEO notification | 241 |
...
| `roboco/services/coroner_engine.py` | `CoronerEngine` — event-only (`open_for_incident`, no `run_cycle`/cron path); incident + transition-history context; `extra_context` merges stranded-specific fields into the incident marker | 257 |
...
| `CoronerEngine.open_for_incident` / `incident_context` | method | coroner_engine.py:76 / 195 | The ONLY way a Coroner cycle opens — called directly from four chokepoints (bounce>=3, cancel-after-work, budget-block, `StrategyEngine`'s `stranded_blocked`), never the cron loop; takes an `extra_context` dict merged into the incident marker (used by the stranded trigger for block_reason/time_blocked/escalation_history); `_ORIGINATORS["coroner"]` is an always-`None` stub that only exists so the dict covers the registry 1:1 |
...
**Strategy flow (dormant watcher).** `Orchestrator._strategy_engine_loop` (created at startup) returns immediately unless `strategy_engine_enabled`; otherwise each `strategy_engine_interval_seconds` it opens a DB context and calls `StrategyEngine.run_cycle` → `assess`. `assess` reads `TaskService.list_in_progress_or_claimed` and `list_long_running_blocked` against `CompanyGoalsService.get()`; if idle-with-goals or stranded-blocked, it sends the CEO an ack-notification via `NotificationService.send_ack_notification`. The engine itself never spends/builds/auto-approves, but each observation kind also triggers its own Board Program before the notification is sent: `idle` calls `_trigger_roadmap_cycle` (`BoardProgramEngine.open_program_cycle("roadmap")`), and `stranded_blocked` calls `_trigger_coroner_incident`, which re-queries `list_long_running_blocked` for the most-stale task and calls `CoronerEngine.open_for_incident(kind="stranded", extra_context={block_reason, time_blocked, escalation_history})` — both helpers are best-effort (any exception is caught and logged, degrading to an "attempted" line) and both fold their outcome string into the same CEO notification body instead of only describing the drift.
...
flowchart TD
    ...
    subgraph StrategyLoop[dormant — strategy_engine_enabled]
        ...
        Assess -->|idle| RoadmapTrigger[_trigger_roadmap_cycle]
        Assess -->|stranded_blocked| CoronerTrigger[_trigger_coroner_incident]
        RoadmapTrigger --> NotifyCEO[CEO notification]
        CoronerTrigger --> NotifyCEO
    end
...
- **Event hooks (bypass the loop entirely):** `TaskService`'s bounce-into-`needs_revision` transition and cancel-after-work path both call `CoronerEngine.open_for_incident` directly (`services/task.py:812` / `:1392`); the orchestrator's budget-block path calls it too (`orchestrator.py:8311`); `StrategyEngine.run_cycle`'s `stranded_blocked` branch calls it a fourth way, through its own `_trigger_coroner_incident` helper (`strategy_engine.py:151`), which re-derives the most-stale stranded task on every strategy-engine tick rather than firing off a single state transition; `ReleaseProposalService.approve`'s publish-success branch calls `WarRoomEngine.open_for_release` (`services/release_proposal.py:330`) alongside the pre-existing `XEngine.draft_release_post` hook.
...
- **The Strategy Engine's `stranded_blocked` → Coroner fold shipped (closing a Phase 2 gap).** The internal design spec (`docs/internal/specs/2026-07-24-board-programs-design.md` §3) proposed both `StrategyEngine` signals becoming Board Program triggers — `idle` → Printer (roadmap) and `stranded_blocked` → Coroner. The `idle`→roadmap half shipped first (Phase 1); the `stranded_blocked`→Coroner half landed later via `StrategyEngine._trigger_coroner_incident` (strategy_engine.py:151), which re-queries `list_long_running_blocked` for the most-stale task and calls `CoronerEngine.open_for_incident(kind="stranded", extra_context=...)`. Coroner is now reachable through four chokepoints (bounce/cancel/budget-block/stranded); a long-stranded blocked task opens an autopsy on the next strategy-engine tick (subject to the usual one-open-autopsy dedup and `program_armed("coroner")` gate) instead of only notifying the CEO.
...
- **CLAUDE.md's "Board Program registry" entry documents the shipped scope accurately, including the `stranded_blocked`→Coroner fold.** Code matches: `program_armed` has no master flag (services/board_programs.py:274), the strategy-engine fold now covers both `idle`→roadmap and `stranded_blocked`→Coroner (strategy_engine.py:108-125). No drift.
