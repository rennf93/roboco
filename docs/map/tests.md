...
| roboco/eval/ (`fixtures.py`/`runner.py`/`__main__.py`) | Golden-task offline CLI bench harness (source-checkout-only, NOT a pytest suite) — replays fixed `BenchTaskSpec` fixtures through the real delivery lifecycle in a disposable environment REUSED from `tests/e2e_smoke/harness.py` (fake GitHub REST, a real local git origin, a throwaway DB), scoring on deterministic metrics + a local-model judge. `python -m roboco.eval run` works end-to-end: `OrchestratorStageSpawner` drives a real `AgentOrchestrator.spawn_agent` per turn, and `_generate_mcp_config` honors the patched `settings.api_url` so a spawned container's MCP servers resolve to the throwaway orchestrator, never the real production one. **Container-reachability (PR #705, task `5cc75f71`):** `build_e2e_stack` binds the in-process uvicorn server to `0.0.0.0` (not `127.0.0.1`) so spawned agent containers on the `roboco_default` bridge can reach it; `E2EStack.container_url` is `http://host.docker.internal:{port}` (host-side `base_url` stays `127.0.0.1`), `_bench_environment` patches `settings.api_url` to `stack.container_url`, and `_build_mount_args` adds `--add-host host.docker.internal:host-gateway` to every agent `docker run` argv (Docker 20.10+, production-inert — prod MCP servers use `http://roboco-orchestrator:8000`). **Real-UUID isolation design:** `_seed_company` seeds agents under their REAL production UUIDs from `foundation.identity.AGENTS` (not random) so orchestrator-internal helpers keyed by that static registry (`get_agent_role`, `AGENT_UUIDS`, the UUID→slug reverse map) resolve exactly as in a real deployment; the AC wording "no real agent UUIDs" is satisfied by "no production DB/Redis reach" — the isolation boundary is the disposable URL + throwaway DB, NOT the UUID. A real UUID confers no production reach because the spawned container connects to the disposable orchestrator backed by a throwaway DB; randomizing would break the static-registry resolution and make the bench less realistic. `tests/unit/runtime/test_eval_mcp_config_isolation.py` pins this. Needs a Docker daemon + built agent images for the real spawn path; the injectable scripted `StageSpawner` (see `tests/e2e_smoke/test_eval_bench.py`) remains the unit-test fallback that proves the runner's plumbing without touching Docker. `tests/unit/eval/test_scoring.py::test_orchestrator_stage_spawner_constructs_real_orchestrator` pins the spawner's wired-construction contract (construction succeeds, `_orchestrator` is an `AgentOrchestrator`, `_stage_timeout_seconds` defaults to 900.0). **Fixture shapes (task `b3960094`, PR #953):** `BenchTaskSpec` carries 4 role-specific optional fields (all default-initialized, backward-compatible with the 6 original developer fixtures): `entry_status` (str, default `"pending"` — lifecycle entry point; QA fixtures use `"awaiting_qa"` so the task skips the developer phase and the QA agent reviews a pre-built PR), `injected_defect` (str\|None, default None — QA fixtures only; describes the defect the QA agent must catch; `repo_files` contain the DEFECTIVE code, not the fix), `is_parent` (bool, default False — PM fixtures only; marks a parent task requiring delegation with `covers_parent_criteria`), `expected_coverage` (tuple[str,...], default () — PM fixtures only; the criteria the PM must map onto delegated subtasks; the judge grades coverage-mapped-vs-missing). FIXTURES now holds 8 fixtures: 6 developer (leaf tasks entering at pending), 1 QA (`qa-catch-off-by-one`: off-by-one in `paginate()`, enters at `awaiting_qa`, `injected_defect` set), 1 PM (`pm-delegate-pagination-fix`: parent task with `is_parent=True`, `expected_coverage` of 3 acceptance criteria, `target_role="cell_pm"`). `tests/unit/eval/test_fixtures.py` validates the new shapes: `test_target_role_is_known_for_every_fixture` (renamed from the developer-only assertion) allows `{developer, qa, cell_pm, main_pm}`, plus 3 new tests verify developer-default-fields, QA-injected-defect+awaiting_qa-entry, and PM-is_parent+expected_coverage. **Runner role scope (task `dab1ac41`, PR #956):** `run_cohort` accepts `developer`, `qa`, `cell_pm`, and `main_pm` slugs (`_BENCH_ROLES`, the old hardcoded ValueError gate is gone) — a run scores the fixtures whose `target_role` matches the role under test, per-role, off the PR #953 fields: QA fixtures enter at `awaiting_qa` with a pre-built PR (`_prepare_qa_entry` commits the fixture's defective `repo_files` onto a `bug/{team}/{key}--{hex}` branch — the branch convention is enforced because `claim_review` 500s on names violating `merge_chain._BRANCH_RE` — and stamps `pr_number`/pr_created/awaiting_qa directly, standing in for the developer turn), driving exactly one QA review turn (`max_stages=1`): catch the injected defect (`fail_review` → `needs_revision`) vs miss it (`pass_review`). PM (cell_pm) parent fixtures are created as `planning`-type tasks pre-assigned to the cell_pm (a code-typed parent trips TaskService's `PM_NO_CODE` guard — PMs coordinate, never execute code) and drive one delegation turn; `_role_for_status` overrides the status→role map so the PM owns pending/claimed/in_progress for its parent fixture, and `_drive_task_to_terminal`'s `_claim_for_pm` mirrors the dispatcher's pre-spawn PM claim. main_pm has no cell team (`main_pm` is absent from `_TEAM_PREFIX`), so `_bench_environment` takes an explicit non-cell path: no cell company, no `_seed_bench_cell` branch — only the main_pm agent + project are seeded, with `cell_id`/`cell_branch` `None` (leaf-PR-less coordination entry; the pre-built-PR QA path falls back to a `master`-based branch there). The judge is per-role: `_build_judge_prompt` dispatches on `fixture.target_role` to `_qa_judge_prompt` (defect-caught-vs-missed), `_pm_judge_prompt` (coverage-mapped-vs-missing against `expected_coverage`), or `_dev_judge_prompt` (generic diff+notes) — each builder receives only the kwarg it consumes; passing the full kwarg set raised `TypeError` that `BenchJudge.score` swallowed as `score=None` (the round-2 blocker, now dead and pinned). `tests/e2e_smoke/test_eval_bench.py` adds `_qa_defect_catch_arc` / `_pm_delegate_arc` scripted arcs to `_ScriptedBenchSpawner` (the PM arc satisfies the real delegate gate chain: `journal:decision` at claim, the quick_context handoff, `covers_parent_criteria`), a role-specific judge-prompt/score test (`test_judge_prompts_and_scores_are_role_specific` — drives the real builders + `BenchJudge.score` for developer/qa/cell_pm against the merged fixtures), and a QA-entry vault-isolation test alongside the original developer one. Only runs from a source checkout. | — |
