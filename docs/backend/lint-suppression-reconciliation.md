...
## Post-audit additions: guard-core 3.12.0 stub gaps (PR #945)

After this audit closed, the guard-core 3.12.0 / fastapi-guard 7.6.0 upgrade introduced 5 new `type: ignore` suppressions (2 in `roboco/security.py`, 3 in `tests/unit/test_security.py`) plus 4 `cast()` calls for tuple/MappingProxyType-vs-list/dict mismatches. These are upstream stub gaps, not silenced debt, and carry waivers in `.roboco/conventions.yml` under `no_lint_suppressions`. Full accounting: [`guard-core-stub-gaps.md`](guard-core-stub-gaps.md).
