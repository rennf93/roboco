...
**Post-audit additions:** suppressions legitimately added after this audit ran are accounted in ["Suppressions added since the audit"](#suppressions-added-since-the-audit) below (currently: the guard-core 3.12.0 stub-gap waivers in `roboco/security.py` / `tests/unit/test_security.py`, task `1073f708`).
...
## Suppressions added since the audit

The 32 above are the audit's closure state, not forever-frozen: new suppressions can be legitimately added after it, each with its own `no_lint_suppressions` waiver in `.roboco/conventions.yml`. This section accounts for those so this doc stays the canonical census (this is where future post-audit suppressions belong — append a subsection, do not edit the reconciliation tables, which describe the audit as it ran).

### guard-core 3.12.0 / fastapi-guard 7.6.0 stub-gap type ignores (task `1073f708`, PR #972, 2026-08-31)

The fastapi-guard 7.6.0 / guard-core 3.12.0 adoption introduced 9 mypy errors whose root cause is upstream stub drift, not RoboCo's code: the stubs simply do not match runtime behavior (a submodule `guard` does not re-export, a Pydantic kwarg the pydantic-mypy plugin omits from the generated `__init__` signature, and collection fields the runtime coerces from mutable `list`/`dict` to immutable types at construction while the stubs keep declaring the mutable type). They were resolved with the smallest honest bridge per error class:

- **`roboco/security.py` — 2 waived `type: ignore` suppressions.** `from guard import status as guard_status  # type: ignore[attr-defined]` (the import path is correct at runtime under guard 7.6.0; `guard.__init__` just never re-exports it) and `behavior_scan_response_body=...  # type: ignore[call-arg]` (the kwarg exists on `SecurityConfig` at runtime; the plugin does not surface it). Waived as one entry under `roboco/security.py` in `.roboco/conventions.yml`. These are NOT silenced debt — they document correct code against wrong stubs; when guard/guard-core ship fixed stubs, both ignores and the waiver should be removed together.
- **4 `cast()` calls instead of suppressions.** `trusted_proxies`, `whitelist`, `threat_ban_config`, and `global_behavior_rules` in `build_security_config()` are wrapped in explicit `typing.cast(...)` — a runtime no-op that documents the expected type where the runtime's coercing constructor and the stub disagree. Casts are not suppressions, so no waiver is owed for them; the convention prefers them wherever a cast can bridge the gap at all.
- **`tests/unit/test_security.py` — 3 waived `type: ignore` suppressions.** One `unreachable` on the `pytest.raises(AttributeError)` immutability-regression body, and two `attr-defined` on `behavior_scan_response_body` accesses (same plugin gap as the call-arg above). Waived under `tests/unit/test_security.py`. Note these sit in `tests/`, which the conventions standard's scanner scope excludes — the audit's own grep surface was `roboco/` + `panel/` — so they register only as waiver entries, not as scanner counts; they are itemized here so the census stays exhaustive.

Zero runtime behavior change by construction (`cast()` returns its second argument; suppressions are annotation-only). All 9 markers carry documented root causes; none is silenced debt.
