...
- `test_cell_and_root_reach_completed_via_gate_and_ceo_approval` — the happy path: `submit_up` -> `claim_gate_review`/`pr_pass` (cell->root gate) -> cell PM `complete` -> `submit_root` -> `claim_gate_review`/`pr_pass` (root->master gate) -> main PM `complete` -> `ceo_approve`. Both the cell and the root carry a real `WorkSessionTable` row seeded `pr_status="open"`: the cell's is flipped to `"merged"` by the real `pr_merge` pre-side-effect `complete()` runs (asserted afterward), and the root's is proven non-circular — `ceo_approve` is asserted to refuse while the session is still `"open"`, then the flip is driven through `_MultiTaskStubGit.pr_merge` (not a literal field write) before a second `ceo_approve` call reaches `completed`.
