...
The Strategy Engine is a **notify-only** background watcher (`ROBOCO_STRATEGY_ENGINE_ENABLED`, default off). Each cycle it `assess()`es the company against its standing goals and emits `StrategyObservation`s — each a `kind`, a `summary`, and a `detail` — for example `idle` (capacity sitting unused) or `stranded_blocked` (work stuck in `blocked`). It never spends, builds, or auto-approves itself, but both observation kinds now also trigger a Board Program: `run_cycle` calls `BoardProgramEngine.open_program_cycle("roadmap")` for `idle` and `CoronerEngine.open_for_incident` for `stranded_blocked` (each enabled/armed + dedup checked in its own engine, so a still-open cycle/autopsy is a harmless no-op) and folds the outcome into the CEO notification instead of only describing the drift. The `stranded_blocked` trigger re-queries `TaskService.list_long_running_blocked` for the most-stale task and calls `CoronerEngine.open_for_incident(kind="stranded", extra_context=...)`, carrying block reason / time blocked / escalation history into the coroner incident marker — this is the fourth of Coroner's event hooks, alongside a bounce past `revision_count>=3`, a cancel-after-work-started, and a budget block (see `docs/rag/architecture/board-programs.md`).
