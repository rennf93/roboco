...
RoboCo's HTTP request layer is protected by `fastapi-guard` (8.0.0, on `guard-core` 4.0.0), implemented in `roboco/security.py` and wired into the app in `roboco/api/app.py`'s `create_app`.
