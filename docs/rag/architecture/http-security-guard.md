...
RoboCo's HTTP request layer is protected by `fastapi-guard` (8.0.0, on `guard-core` 4.0.1), implemented in `roboco/security.py` and wired into the app in `roboco/api/app.py`'s `create_app`.
...
The mesh stays on this global whitelist, since it also carries the panel container, the SDK sidecar, nginx, and both bridges, and guard-core's global `whitelist` is EXCLUSIVE (a non-empty whitelist refuses any non-member outright), so the mesh cannot be pulled off it without breaking every other container on the same docker network. Instead, the agent gateway (every endpoint in `roboco/api/routes/v1/do.py` and `flow_*.py`, only agents call these, confirmed by grepping the panel for `/api/v1/`) carries its own route-level allowlist via `mesh_scanned` (`roboco/security.py`), applied to all 102 gateway endpoints. `guard_deco.require_ip(whitelist=list(_MESH_ROUTE_ALLOWLIST))` sets `route_config.ip_whitelist`, which makes guard-core's `IpSecurityCheck` skip the GLOBAL whitelist for that route (`_route_overrides_ip_lists` / `skip_ip_lists`) and stamp `request.state.is_whitelisted = False` even though the mesh IP still passes the route's own allowlist, and since `SuspiciousActivityCheck` only skips when `is_whitelisted`, the WAF actually runs against agent traffic on these routes for the first time. `mesh_scanned` also applies `bypass(["ip_ban", "rate_limit"])` on the same routes: the ask was WAF scanning, not a new way for an HMAC-authenticated agent to 429, and the route-level `rate_limit` bypass short-circuits `RateLimitCheck.check` before its endpoint/route/geo checks AND its trailing global-bucket check ever run, so this also protects the mesh from the GLOBAL 120/60 bucket on these routes, not just their own per-route `@guard_deco.rate_limit(...)` decorators. Any other caller (tailnet, a forwarded LAN client, the public internet) is judged solely against the route's mesh-only allowlist there and is blocked outright (403), same exclusivity as the global list. Every other route (panel, `/api/optimal/*`, `/health`, ...) is unchanged: the mesh stays fully exempt there.

Ban safety is separate and mandatory: `_THREAT_BAN_CONFIG` bans some categories (`cmd_injection`) on a single hit, and a ban overrides the whitelist on EVERY route, so one WAF false positive on a gateway route could otherwise IP-ban the whole agent container fleet-wide, including on routes the gateway policy never touches. `roboco/security.py` patches `IPBanManager.ban_ip` at import time (the single seam the WAF's threat escalation records a ban through; the rate-limit auto-ban shares the seam too but can no longer even fire on a `mesh_scanned` route now that `rate_limit` is bypassed there): for any IP or CIDR range overlapping `_MESH_ROUTE_ALLOWLIST` (`ipaddress.ip_network(ip, strict=False).overlaps(...)`, so a range ban that merely overlaps the mesh is refused too, not just an exact member address) it logs a warning and returns without banning, everyone else is unaffected. The mesh is blocked per request on the gateway route policy above, never banned fleet-wide, a compromised container is filtered call by call, not locked out. guard-core 4.0.0 also independently refuses to ban any IP overlapping `trusted_proxies` (its own self-DoS guard), which today is identical to `_MESH_ROUTE_ALLOWLIST`, so this is belt-and-suspenders rather than the only thing standing between a WAF hit and a fleet-wide lockout. guard-core 4.0.1's `ban_ip` returns a bool that the WAF threshold ban, the identity-violation escalation, and the behavioral ban action all read, and the wrapper propagates it (False on a mesh refusal), so an applied ban is logged and reported as `banned` rather than `tracked`, and on the WAF path answers 403 "IP has been banned" rather than 400 "Suspicious activity detected" (a case `test_waf_threshold_ban_reports_applied_when_external_reaches_waf` pins with an emptied whitelist, since the deployed exclusive whitelist blocks an external client before the WAF).

## `trusted_proxies` Must Track the Whitelist

`build_security_config` passes guard-core a second, easily-confused list alongside the whitelist: `trusted_proxies` (the addresses guard treats as *proxy hops* when it walks `X-Forwarded-For` to depth `trusted_proxy_depth`). It is NOT the whitelist (which decides who skips WAF/ban/rate-limit), but the two MUST stay in lockstep, and on this deploy they're identical: `127.0.0.1`, `::1`, `172.16.0.0/12` (loopback plus docker's default bridge pool). `trusted_proxies`, `_INTERNAL_NETWORKS`, and the gateway's `_MESH_ROUTE_ALLOWLIST` all derive from one `_MESH_NETWORKS` constant in `roboco/security.py` now, so the three can no longer drift out of sync the way three hand-typed copies could.
...
## Telemetry: Last-Known Dynamic Rules (guard-core 4.0.0)

With telemetry armed, `_agent_kwargs()` also sets `enable_dynamic_rules=True` and passes `ROBOCO_GUARD_DYNAMIC_RULES_CACHE_PATH` through as `dynamic_rules_cache_path`. guard-core 4.0.0 persists every successfully applied dynamic-rules payload to redis (primary, under the `roboco:guard:` prefix) and, when the path is set, to an atomically written JSON file (fallback). At startup it hydrates once, redis first then the file, so a restart while the SaaS or redis is unreachable restores the last applied rules instead of base config. An empty path means redis only. The build compose points it at `/data/logs/guard-dynamic-rules.json`, the persisted logs mount.

`guard-agent` is NOT a dependency (CEO opt-out, 2026-09-02) and telemetry is carried OFF in every compose file. If it were armed with a key anyway, fastapi-guard would catch `AgentPackageNotInstalledError` and continue agent-less (roboco sets `agent_strict=False`), so telemetry and dynamic rules would silently no-op until the package is added. With no API key, `_agent_kwargs()` stays empty (guard-core rejects `enable_agent=True` without `agent_api_key` at construction, and this config is built at import), so the agent is never configured and nothing leaves the box. `test_agent_kwargs_empty_when_armed_without_key` pins that. `tests/unit/test_security.py::test_security_config_builds_agent_config_when_telemetry_armed` pins both.

## WebSocket Handshake Gate

`SecurityMiddleware` is a `BaseHTTPMiddleware` and never dispatches for websocket scopes, so the `/ws` entry in `exclude_paths` never protected anything: the `/ws/*` streams in `roboco/api/websocket.py` had only the panel-token check. `guard_ws` in `roboco/api/websocket.py` (fastapi-guard's `guard_websocket` behind the `guard_enabled` flag) is a router-level dependency on that router. It runs guard's IP-ban check and the exclusive allowlist at the handshake and refuses with close code 1008 before `accept()`. `ClientIpResolutionMiddleware` stamps websocket scopes too, so a host-proxied tailnet client resolves to its real address and an external client behind the nginx hop is judged by the forwarded IP, never by the whitelisted peer. With a non-empty whitelist guard skips its per-IP websocket rate limit, which is fine: only whitelisted peers ever reach it. If `SecurityMiddleware` never mounted (`apply_guard` is best-effort), `guard_ws` logs an error and fails open, matching the unguarded HTTP side instead of refusing every stream. `tests/unit/test_security_middleware.py::TestWebSocketGate` pins all of this.

## Rate-Limit Auto-Ban

`enable_rate_limit_auto_ban=True` feeds every active-mode 429 into the same ban engine as the WAF, under the `rate_limit` pseudo-category of `_THREAT_BAN_CONFIG` (20 violations, 1h ban). Before this only `usage_monitor(action="ban")` routes escalated and every other rate-limited route re-429'd forever. Whitelisted peers never reach the rate limiter and passive mode suppresses the count, so only external repeat offenders are banned.
