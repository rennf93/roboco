...
**Date:** 2026-09-05 **Task:** bfb48210, 13af9490, e18d8f49 **PR:** #1015; post-publish reachability fix in PR #1028 (task `13af9490`); task-status-polling rewrite in task `e18d8f49` **Files:** `panel/src/lib/api/release.ts`, `panel/src/components/dashboard/release-proposal-card.tsx`
...
- If the polled task instead reaches a terminal non-completed status (`cancelled`), the pointer will never resolve to a certificate, so it's cleared from `localStorage` and local state — this is done as a render-time state adjustment (comparing against a `useRef`-tracked previous status), not inside a `useEffect`, to avoid the cascading-render anti-pattern the React Compiler ESLint rule (`react-hooks/set-state-in-effect`) flags for a `setState` call synchronous inside an effect body.
