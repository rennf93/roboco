...
import { spackleApi } from "@/lib/api";
import type { SpackleCycle, GapFillItem } from "@/lib/api/spackle";
import {
  Card,
  CardContent,
  CardDescription,
...
function itemStatusBadge(item: GapFillItem) {
...
// One gap-fill item row: details + per-item approve/reject (proposed only).
function GapFillItemRow({
  taskId,
  item,
  onApprove,
  onReject,
  ...
  return (
    <div className="rounded-lg border p-4 transition-colors hover:bg-muted/50">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <HelpTip label="The project (repository) this gap-fill item targets">
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Half-shipped surface area the Product Owner audited — evidence-backed
          gap-fill drafts, each reviewed individually.
        </CardDescription>
      </CardHeader>
...
// CEO queue for the Product Owner's held Spackle cycles. Hidden when no
// cycle has been authored yet (mirrors PestControlReviewQueue).
export function SpackleReviewQueue({ className }: { className?: string }) {
  ...
  const { data: cycles, isLoading } = useQuery({
    ...
    queryFn: () => spackleApi.listCycles(),
    refetchInterval: 30000,
  });

  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["spackle", "cycles"] });

  const approveMutation = useMutation({
    mutationFn: ({ taskId, itemId }: { taskId: string; itemId: string }) =>
      spackleApi.approveItem(taskId, itemId),
    onSuccess: (result) => {
      invalidate();
      if (
  ...
  const rejectMutation = useMutation({
    mutationFn: ({
      ...
      itemId: string;
      reason: string;
    }) => spackleApi.rejectItem(taskId, itemId, reason),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <>
      ...
      <Dialog open={!!rejecting} onOpenChange={() => closeReject()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject gap-fill item</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-2">
            <Label htmlFor="spackle-reject-reason">Reason</Label>
            <Textarea
              id="spackle-reject-reason"
