...
import type { MirrorCycle, MessagingFixItem } from "@/lib/api/mirror";
import {
  Card,
  CardContent,
  CardDescription,
...
interface RejectTarget {
  taskId: string;
  item: MessagingFixItem;
}

function itemStatusBadge(item: MessagingFixItem) {
  if (item.status === "approved") {
    return (
      <HelpTip label="Materialized as a BACKLOG docs task">
...
// One messaging-fix item row: details + per-item approve/reject (proposed only).
function MessagingFixItemRow({
  taskId,
  item,
  onApprove,
  onReject,
  ...
  return (
    <div className="rounded-lg border p-4 transition-colors hover:bg-muted/50">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <HelpTip label="The project (repository) this messaging-fix item targets">
          <Badge variant="outline">{item.project_slug}</Badge>
        </HelpTip>
        <HelpTip
          label={`Priority P${item.priority} — ${item.priority === 0 ? "critical" : item.priority === 1 ? "high" : item.priority === 2 ? "medium" : "low"}`}
        >
      ...
      {isProposed && (
        <div className="mt-3 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
          ...
          <HelpTip label="Materializes this item as a BACKLOG docs task — needs normal PM activation to start">
            <Button
              size="sm"
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Messaging drift the Head of Marketing audited against the charter and
          shipped reality — evidence-backed docs-fix drafts, each reviewed
          individually.
        </CardDescription>
      </CardHeader>
...
// CEO queue for the Head of Marketing's held Mirror cycles. Hidden when no
// cycle has been authored yet (mirrors SpackleReviewQueue).
export function MirrorReviewQueue({ className }: { className?: string }) {
  ...
  const { data: cycles, isLoading } = useQuery({
    queryKey: ["mirror", "cycles"],
    queryFn: () => mirrorApi.listCycles(),
    refetchInterval: 30000,
  });

  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["mirror", "cycles"] });

  const approveMutation = useMutation({
    mutationFn: ({ taskId, itemId }: { taskId: string; itemId: string }) =>
      mirrorApi.approveItem(taskId, itemId),
    onSuccess: (result) => {
      invalidate();
      if (
  ...
  const rejectMutation = useMutation({
    mutationFn: ({
      ...
      itemId: string;
      reason: string;
    }) => mirrorApi.rejectItem(taskId, itemId, reason),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <>
      ...
      <Dialog open={!!rejecting} onOpenChange={() => closeReject()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject messaging-fix item</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-2">
            <Label htmlFor="mirror-reject-reason">Reason</Label>
            <Textarea
              id="mirror-reject-reason"
              placeholder="e.g. already tracked elsewhere; not worth the churn right now..."
              value={reason}
