...
import { dogfoodApi } from "@/lib/api";
import type { DogfoodCycle, FrictionFixItem } from "@/lib/api/dogfood";
import {
  Card,
  CardContent,
  CardDescription,
...
interface RejectTarget {
  taskId: string;
  item: FrictionFixItem;
}

function itemStatusBadge(item: FrictionFixItem) {
...
// One friction-fix item row: details + per-item approve/reject (proposed only).
function FrictionFixItemRow({
  taskId,
  item,
  onApprove,
  onReject,
  ...
  return (
    <div className="rounded-lg border p-4 transition-colors hover:bg-muted/50">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <HelpTip label="The project (repository) this friction item targets">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        <span className="font-semibold">Walked path:</span> {item.evidence}
      </p>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          UX friction the Product Owner filed while walking the product as a
          user — evidence-backed drafts, each reviewed individually.
        </CardDescription>
      </CardHeader>
...
// CEO queue for the Product Owner's held Dogfood cycles. Hidden when no
// cycle has been authored yet (mirrors SpackleReviewQueue).
export function DogfoodReviewQueue({ className }: { className?: string }) {
  ...
  const { data: cycles, isLoading } = useQuery({
    ...
    queryFn: () => dogfoodApi.listCycles(),
    refetchInterval: 30000,
  });

  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["dogfood", "cycles"] });

  const approveMutation = useMutation({
    mutationFn: ({ taskId, itemId }: { taskId: string; itemId: string }) =>
      dogfoodApi.approveItem(taskId, itemId),
    onSuccess: (result) => {
      invalidate();
      if (
  ...
  const rejectMutation = useMutation({
    mutationFn: ({
      ...
      itemId: string;
      reason: string;
    }) => dogfoodApi.rejectItem(taskId, itemId, reason),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <>
      ...
      <Dialog open={!!rejecting} onOpenChange={() => closeReject()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reject friction-fix item</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-2">
            <Label htmlFor="dogfood-reject-reason">Reason</Label>
            <Textarea
              id="dogfood-reject-reason"
