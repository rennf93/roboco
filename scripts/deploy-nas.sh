...
# Stop the previous color BEFORE restarting nginx: nginx resolves the
# upstream hostnames in the include at config load, so a restart after the
# stop is what re-resolves DNS to the new color cleanly (a hot reload kept
# stale upstreams and tripped post-deploy, 2026-09-17).
...
if ! wait_healthy roboco-nginx; then
  echo "[deploy] FATAL: nginx unhealthy after the flip. Recovery: fix or restore" >&2
  echo "[deploy] front/active-upstreams.conf (a .prev copy holds the previous color's" >&2
  echo "[deploy] upstreams), then: docker compose -f docker-compose.yaml up -d --force-recreate nginx" >&2
  exit 1
fi
