"""Migration 095/096 tests: modelprovider_openrouter + seed_openrouter_provider.

Verifies the post-upgrade state and exercises the downgrade SQL ordering,
mirroring ``test_migration_091_seed_kimi_provider.py``'s own shape with ONE
decisive contrast: OpenRouter's row is seeded ``enabled=false`` (the GROK
posture, not Kimi's). Unlike the subscription-CLI providers there IS a
key-collection step gating the row: the Fernet-encrypted OpenRouter API key
set via PUT /providers/openrouter-key, which encrypts + enables in the same
transaction.

NOT a real alembic round-trip: the suite builds the test DB via
Base.metadata.create_all (see conftest). Migrations 095/096's
...
095 enum change is exercised as a real round-trip against the
...
# The exact INSERT from migration 096's upgrade() (seed enabled=false).
_INSERT_SQL = text(
    """
...
@pytest.mark.asyncio
async def test_migration_096_upgrade_insert_contract(
    db_session: AsyncSession,
) -> None:
    """The upgrade INSERT SQL seeds the OpenRouter row DISABLED with no stored
    ...
    result = await db_session.execute(
        text("SELECT id FROM provider_configs WHERE name = 'OpenRouter'")
    )
    assert len(list(result)) == 1, (
        "Expected exactly one 'OpenRouter' row after two INSERT "
        "executions; ON CONFLICT DO NOTHING must prevent duplicates."
    )


@pytest.mark.asyncio
async def test_migration_096_downgrade_deletes_assignments_before_config(
    db_session: AsyncSession,
) -> None:
    ...
    await db_session.execute(
        text(
            ...
            ")"
        ).bindparams(name=openrouter.name)
    )
...
@pytest.mark.asyncio
async def test_migration_095_openrouter_enum_round_trip(
    db_session: AsyncSession,
) -> None:
    """The ``modelprovider`` PG enum carries ``openrouter`` (migration 095)
    and the value round-trips through the ORM."""
    # Raw PG-level check: the enum type itself contains the label.
    result = await db_session.execute(
        text("SELECT 'openrouter'::modelprovider = 'openrouter'")
    )
    assert result.scalar_one() is True

    # ORM round-trip: a row typed OPENROUTER survives write + read.
    suffix = uuid4().hex[:8]
    openrouter = ProviderConfigTable(
        name=f"OpenRouter-enum-{suffix}",
        type=ModelProvider.OPENROUTER,
        enabled=False,
    )
    db_session.add(openrouter)
    ...
    reloaded = await db_session.get(ProviderConfigTable, openrouter.id)
    assert reloaded is not None
    assert reloaded.type is ModelProvider.OPENROUTER
