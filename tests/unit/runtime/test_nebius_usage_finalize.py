"""NEBIUS agents capture real 4-bucket token usage + metered cost from
their captured ``usage.json`` (see ``nebius_cli_usage``): finalize must
return the genuine split (the codex/kimi shape) and must attribute spend from
Token Factory's own metered ``cost`` field, never by re-pricing tokens through
the static pricing table (it cannot know Nebius's live catalog).
"""
...
def test_nebius_usage_returns_real_split(
    ...
    monkeypatch.setattr(
        orch, "_nebius_usage_json", lambda _aid: json.loads(usage.read_text())
    )

    assert orch._nebius_usage_tokens("be-dev-1") == (300, 130, 30, 7)


def test_nebius_usage_zero_when_store_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    monkeypatch.setattr(orch, "_nebius_usage_json", lambda _aid: None)
    assert orch._nebius_usage_tokens("be-dev-1") == (0, 0, 0, 0)


def test_nebius_usage_cost_reads_metered_cost(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The spend IS usage.json's metered ``cost_usd`` (never re-priced)."""
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    monkeypatch.setattr(orch, "_nebius_usage_json", lambda _aid: _usage_payload())
    assert orch._nebius_usage_cost("be-dev-1") == pytest.approx(_METERED_COST)


def test_nebius_usage_cost_none_when_store_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    monkeypatch.setattr(orch, "_nebius_usage_json", lambda _aid: None)
    assert orch._nebius_usage_cost("be-dev-1") is None


@pytest.mark.asyncio
async def test_resolve_final_usage_routes_nebius_to_usage_json(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    monkeypatch.setattr(
    ...
    orch._instances = {"be-dev-1": _nebius_instance()}

    assert await orch._resolve_final_token_usage("be-dev-1") == (12, 34, 5, 1)


@pytest.mark.asyncio
async def test_resolve_final_turns_tools_nebius_has_no_turn_signal() -> None:
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    orch._instances = {"be-dev-1": _nebius_instance()}

    # Nebius's capture has no per-turn signal (the grok/gemini shape).
    assert await orch._resolve_final_turns_tools("be-dev-1") == (0, 0)


@pytest.mark.asyncio
async def test_resolve_active_tokens_routes_nebius_to_usage_json(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    monkeypatch.setattr(
...
def test_nebius_usage_dir_branches_compose_vs_local(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(usage_root_mod, "PROJECT_HOST_PATH", "")
    local = AgentOrchestrator._nebius_usage_dir("be-dev-1")
    assert "roboco-nebius-usage" in str(local)
...
def test_nebius_usage_dir_rejects_path_traversal(bad: str) -> None:
    with pytest.raises(ValueError, match="unsafe agent id"):
        AgentOrchestrator._nebius_usage_dir(bad)


def test_nebius_usage_json_reads_the_real_local_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(usage_root_mod, "PROJECT_HOST_PATH", "")
    monkeypatch.setattr(tempfile, "gettempdir", lambda: str(tmp_path))
    udir = tmp_path / "roboco-nebius-usage" / "be-dev-1"
    udir.mkdir(parents=True)
    _write_usage(udir / "usage.json", tokens_input=55, tokens_output=10)
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    assert orch._nebius_usage_tokens("be-dev-1") == (55, 10, 0, 0)
...
def _nebius_session_instance(agent_id: str = "be-dev-1") -> AgentInstance:
    cfg = type(
        "C",
        (),
        {
...
async def test_finalize_spawn_session_maps_buckets_and_metered_cost(
    ...
    orch = AgentOrchestrator.__new__(AgentOrchestrator)
    orch._instances = {"be-dev-1": _nebius_session_instance()}
    monkeypatch.setattr(orch, "_nebius_usage_json", lambda _aid: _usage_payload())
