"""nebius_cli_usage - sum step_finish events' part.tokens + part.cost from
...
Token Factory's metered ``cost`` field, NOT the static ``_PRICING`` table (no
Nebius rows by design).
"""
...
from roboco.llm.providers import nebius_cli_usage as ou
...
def test_main_warns_when_run_log_env_missing(
    ...
    monkeypatch.delenv("ROBOCO_NEBIUS_RUN_LOG", raising=False)
    with caplog.at_level("WARNING", logger="roboco.llm.providers.nebius_cli_usage"):
        assert ou.main() == 0
    assert any("ROBOCO_NEBIUS_RUN_LOG" in r.message for r in caplog.records)
