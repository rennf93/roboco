"""nebius_cli_config - opencode.json rendering (agent block + permission
...
from roboco.llm.providers import nebius_cli_config as oc
...
# the rendered provider.nebius block, not opencode's built-in anthropic
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# render_provider_block - Token Factory endpoint + env name + npm package
# ---------------------------------------------------------------------------


def test_render_provider_block_has_api_env_npm() -> None:
    block = oc.render_provider_block("https://api.tokenfactory.nebius.com/v1")
    provider = block["nebius"]
    assert provider["api"] == "https://api.tokenfactory.nebius.com/v1"
    assert provider["name"] == "Nebius"
    assert "NEBIUS_API_KEY" in provider["env"]
    assert provider["npm"] == oc._NEBIUS_PROVIDER_NPM
    # Token Factory speaks the generic OpenAI-compatible protocol, so the
    # transport is opencode's generic AI SDK package (not a vendor one).
...
def test_render_config_has_schema_agent_provider_mcp() -> None:
    ...
    assert "nebius" in config["provider"]
...
def test_is_valid_true_when_key_set(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("NEBIUS_API_KEY", "nebius-abc123")
...
def test_main_check_flag_passes_when_key_set(
    ...
    config_path = tmp_path / "opencode.json"
    monkeypatch.setattr(oc, "OPENCODE_CONFIG_PATH", config_path)
    monkeypatch.setenv("NEBIUS_API_KEY", "nebius-abc")
