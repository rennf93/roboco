"""nebius_cli_sniff - classify an opencode run from ONLY its machine-relevant
text.
...
from roboco.llm.providers import nebius_cli_sniff as sniff
...
# ---------------------------------------------------------------------------
# error.data.statusCode - the live OpenAI-compatible shape's only
# classifiable field
# ---------------------------------------------------------------------------
...
def test_live_apierror_401_shape_classifies_auth(tmp_path: Path) -> None:
    """The live-verified OpenAI-compatible 401: the transport error is wrapped
    under a generic "APIError" name with a message matching no auth pattern -
    only the structured statusCode classifies the run."""
