...
- The headline integration built for this hackathon (2026-09-13/14) is **Nebius Token Factory as a first-class LLM provider** for the entire agent fleet, with **NVIDIA's open-source Nemotron 3 Super (`nvidia/nemotron-3-super-120b`) as the fleet-wide default model by construction**, not as an option the operator has to find.
...
**Token Factory Sandboxes is the identified integration for the track's signature product.** Today the fleet's code execution runs inside RoboCo's own hardened agent containers (bounded tool surface, command/secret-exfiltration guards, per-agent cost caps, and an existing on-demand sandbox-subsystem that provisions isolated dev Postgres/Redis/Mongo sidecars per project). The planned build routes the "run and test code" leg of the agent loop through a Token Factory Sandbox: the QA agent's test-run workload executes in a Token Factory-managed sandbox instead of the agent container's local shell, with the same evidence trail (gateway verb envelope, usage metering) as every other RoboCo action. This section will be updated with the integration's specifics as it lands inside the submission window.
