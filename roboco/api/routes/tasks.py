...
async def _apply_forced_status_override(req: _StatusOverride) -> TaskTable:
    ...
    # Admin-complete must merge-or-refuse: completing a task whose PR is
    # still OPEN strands its commits unmerged (bit the CEO twice live,
    # 2026-07-02). Checked before the generic hatch text so the refusal
    # names the PR and the consequence instead of a vague gate message.
    if req.new_status == TaskStatus.COMPLETED and not req.force:
