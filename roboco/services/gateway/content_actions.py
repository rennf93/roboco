...
class ContentActions:
    ...
    async def evidence(
        ...
        *,
        ...
        The git diff/fetch segment and the DB-reads segment run concurrently
        via ``asyncio.gather``, and the whole evidence-assembly is wrapped in
        a bounded ``evidence_assembly_timeout_seconds`` timeout that returns a
        structured ``gateway_timeout`` envelope on trip instead of hitting the
        outer 120s server-side rollback.
        """
        t = await self.task.get(task_id)
        if t is None:
            return Envelope.not_found(message=f"task {task_id} not found")
        ...
        try:
            (
                ...
                asyncio.gather(
                    self._evidence_git_and_fetch(t, agent_id),
                    self._evidence_db_reads(task_id),
                ),
                timeout=timeout,
            )
        except TimeoutError:
            return Envelope.gateway_timeout(
                ...
                remediate=(
                    "retry evidence(task_id=...) after a short wait; if it "
                    "keeps timing out the repo may be too large or a remote "
                    "fetch is stalled"
                ),
                task_id=str(task_id),
                context_briefing={},
    ...
    async def _evidence_git_and_fetch(
        self, t: Any, agent_id: UUID
    ) -> tuple[str, list[str]]:
        """Git diff/fetch segment of evidence() — uses ``diff_and_files``."""
        if not t.branch_name:
            return "", []
        git_start = time.monotonic()
        diff, files_changed = await self.git.diff_and_files(
    ...
    async def _evidence_db_reads(self, task_id: UUID) -> tuple[Any, Any, list[Any]]:
        """DB-reads segment of evidence() — runs concurrently with git work.

        Returns ``(journal_highlights, parent_context, open_findings)``.
        Sequential on purpose: all reads share the request-scoped AsyncSession
        and asyncpg can't serve two in-flight queries on one connection.
        """
