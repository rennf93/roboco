...
class ContentActions:
    ...
    async def _evidence_db_reads(
        self, task_id: UUID
    ) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[Any]]:
        """The three DB reads behind evidence(), run sequentially as ONE
        coroutine.

        They stay sequential AMONG THEMSELVES on purpose: they all read
        through the request-scoped ``self.evidence_repo`` / ``self.task.
        session``, a single ``AsyncSession`` bound to one DBAPI connection —
        asyncpg cannot serve two in-flight queries on the same connection at
        once (confirmed against a live Postgres while building this fix:
        SQLAlchemy raises ``IllegalStateChangeError: ... concurrent
        operations are not permitted``; opening a second ad-hoc session per
        read would also silently diverge from the request's transaction and
        break every existing test that injects a mocked ``evidence_repo``).
        Bundling them as one coroutine lets ``evidence()`` still
        ``asyncio.gather`` this WHOLE batch against the independent git
        diff/fetch — the actual "parallelize the independent reads" win,
        without the same-connection hazard. Mirrors
        ``qa.QAMixin._qa_db_reads``.
        """
        t0 = time.monotonic()
        ...
        logger.info(
            "evidence db reads timing",
            task_id=str(task_id),
            db_reads_ms=round((time.monotonic() - t0) * 1000.0),
        )
        return journal_highlights, parent_context, open_findings
    ...
    async def evidence(
        ...
        *,
        ...
        commit's delta, so reviewers see the full multi-commit change set. Both
        come from the single combined ``git.diff_and_files`` call (one
        workspace/base resolution instead of two) and the assembly work is
        bounded by ``settings.evidence_assembly_timeout_seconds`` — well under
        the outer gateway verb budget — so a genuinely slow git fetch or DB
        read returns a structured, named timeout instead of a bare rollback.
        """
        t = await self.task.get(task_id)
        if t is None:
            return Envelope.not_found(message=f"task {task_id} not found")
        ...
        async def _git_diff() -> tuple[str, list[str]]:
            if not t.branch_name:
                return "", []
            git_t0 = time.monotonic()
            diff_text, files = await self.git.diff_and_files(
                branch_name=t.branch_name, actor_agent_id=agent_id
            )
            logger.info(
                "evidence git diff/fetch timing",
                task_id=str(task_id),
                git_diff_and_fetch_ms=round((time.monotonic() - git_t0) * 1000.0),
            )
        ...
        try:
            (
                ...
                asyncio.gather(_git_diff(), self._evidence_db_reads(task_id)),
                timeout=timeout,
            )
        except TimeoutError:
            return Envelope.gateway_timeout(
                ...
                remediate=(
                    "retry evidence(task_id); a persistent timeout means the "
                    "underlying git fetch or DB read is genuinely slow — "
                    "escalate to your PM"
                ),
                context_briefing={},
            )
