...
``usage.json`` the orchestrator reads back at finalize. It is the Nebius twin
of :mod:`roboco.llm.providers.openrouter_cli_usage` - same event shape, same
4-bucket mapping, different env-var names.

**Cost comes from Token Factory's metered ``cost`` field, NOT the static
``_PRICING`` table.** Nebius charges per-request and reports the charge in
the event; RoboCo attributes spend directly from that field
(:data:`roboco.billing.pricing._PRICING` has NO Nebius rows by design -
...
The 4-bucket mapping (parity with kimi/codex's usage.json shape):
  ...
  * ``tokens_output``     = ``tokens.output`` + ``tokens.reasoning``
    (completion incl. reasoning - OpenAI-compatible APIs price reasoning as
    completion)
  * ``tokens_cache_read``  = ``tokens.cache.read``
  * ``tokens_cache_write`` = ``tokens.cache.write``

The agent entrypoint runs ``python -m roboco.llm.providers.nebius_cli_usage``
...
USAGE_OUT_PATH = Path(
    os.environ.get("ROBOCO_NEBIUS_USAGE_FILE")
...
def _cost_from_step_finish(event: dict[str, Any]) -> float | None:
    ...
    The cost lives under ``event["part"]["cost"]`` (Token Factory's metered
...
def capture_run_usage(
    *,
    ...
    :func:`roboco.billing.pricing.calculate_cost` - Nebius has no
...
def main() -> int:
    ...
    run_log = os.environ.get("ROBOCO_NEBIUS_RUN_LOG", "")
    if not run_log:
        logger.warning("ROBOCO_NEBIUS_RUN_LOG not set; usage will read 0")
        return 0
