...
run's output to tell a Nebius Token Factory rate-limit/quota error or an auth
failure apart from any other error. Sniffing the FULL ``--format json``
stream is unsafe: the model's own on-topic prose can false-positive by
construction - this repo's own role prompts use words like "quota-limited",
and a commit hash or item id can contain the substring "429" or "401"
...
fix, and is the Nebius twin of
:mod:`roboco.llm.providers.openrouter_cli_sniff`).
...
Patterns (the codex/kimi-proven word-boundaried digit guard, plus the
OpenAI-compatible error vocabulary Token Factory returns):
  - rate-limit: a bare ``\\b429\\b``, "rate limit", "too many requests",
    "overloaded", "quota".
  - auth failure: a bare ``\\b401\\b``, "invalid api key", "unauthorized",
    "authentication".

The structured ``error.data.statusCode`` matters live: an OpenAI-compatible
401 response can wrap the transport error under a generic ``"APIError"`` name
whose message ("User not found.") matches no auth pattern above - the
extracted status code is what classifies the run.

The entrypoint calls this as ``python -m roboco.llm.providers.nebius_cli_sniff
...
the Nebius provider identically to grok/codex/gemini/kimi/openrouter.
"""
...
def _status_code_text(status: Any) -> str | None:
    ...
    Accepts the int the API sends (401 / 429) or a numeric string; bools
...
def _dict_error_text(error: dict[str, Any]) -> str | None:
    ...
    ``data.statusCode`` is appended too: an OpenAI-compatible 401 response can
    wrap the transport error under a generic ``"APIError"`` name carrying a
    message ("User not found.") that matches no auth pattern - the structured
    status code is the only machine-classifiable field on that shape (appended
...
def _error_text_from_event(event: dict[str, Any]) -> str | None:
    ...
    classifies correctly; the statusCode classifies the live OpenAI-compatible
